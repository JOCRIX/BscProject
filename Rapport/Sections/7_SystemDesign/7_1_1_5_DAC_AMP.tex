...
This near instability can be explained from the output stage. It is assumed that the output impedance of the push-pull output is \SIQ{0}{\ohm}. Furthermore the frequencies of instability are very high, roughly \SIQ{30}{\mega\hertz}, from figure \refq{fig_7_1_1_5_DAC_POWER_AMP_SIM_150PF}, this will result in the capacitance $C_\pi$ being very close to a short circuit. For simplicity $C_\pi$ is assumed a short circuit based on the frequncies of interrest, this also results in gm being of no interrest. This allows the AB stage to be simplified as in figure \refq{fig_7_1_1_5_DAC_PWR_AMP_PZ} for high frequencies. 
...
Assuming $Z_\pi$ to be 0 the transferfunction is as in equation \refq{eq_DAC_PWR_1}. Note that the biasing resistors $R_b$ have been assumed insignificant.
...
One interesting thing to note from equation \refq{eq_DAC_PWR_1} is that the pole and zero will change depending on the choise of range resistor. Simulations show that the system is most sensitive to instability at loads around \SIQ{150}{\pico\farad}. The larger the range resistor, the less impact the other resistors will have on the pole-zero placement. At a range resistor value of \SIQ{1}{\kilo\ohm} the pole and zero practically cancles. This indicates that the system will always be stable when range resistors at and above \SIQ{1}{\kilo\ohm} are used. 
